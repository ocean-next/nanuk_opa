MODULE sbccpl
   ...
   LOGICAL, PARAMETER, PUBLIC :: l_vect_exchg_en = .FALSE.  !EO: set to .TRUE. if all exchanges of vector fields use eastward-northward orientation
   ...
   INTEGER, PARAMETER ::   jps_fice2  = 26            ! ice fraction sent to OPA (by neXtSIM when doing neXtSIM-OPA coupling)
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sbccpl.F90 6963 2016-09-30 12:40:04Z clem $
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_cpl in configuration namelist', lwp )
      ...
      ENDIF
      !                                                      ! ------------------------------- !
      !                                                      !   OPA-neXtSIM coupling - rcv by opa !
      !                                                      ! ------------------------------- !
      srcv(jpr_sflx)%clname = 'O_SFLX'
      srcv(jpr_fice)%clname = 'RIceFrc'
      !
      IF( nn_components == jp_iam_opa ) THEN    ! OPA coupled to neXtSIM via OASIS: force received field by OPA (sent by neXtSIM)
         srcv(:)%laction = .FALSE.   ! force default definition in case of opa <-> neXtSIM coupling
         srcv(:)%clgrid  = 'T'       ! force default definition in case of opa <-> neXtSIM coupling
         srcv(:)%nsgn    = 1.        ! force default definition in case of opa <-> neXtSIM coupling
         ...
         IF(lwp) THEN                        ! control print
            WRITE(numout,*)
            WRITE(numout,*)'               Special conditions for neXtSIM-OPA coupling  '
            WRITE(numout,*)'               OPA component  '
            WRITE(numout,*)
            WRITE(numout,*)'  received fields from neXtSIM component '
      ...
      IF( nn_components == jp_iam_opa ) THEN
         ssnd(:)%laction = .FALSE.   ! force default definition in case of opa <-> neXtSIM coupling
         ...
         !EO:
         IF( l_vect_exchg_en ) THEN
            sn_snd_crt%clvor = 'eastward-northward'
         ELSE
         ...
         IF(lwp) THEN                        ! control print
            WRITE(numout,*)
            WRITE(numout,*)'  sent fields to neXtSIM component '
   ...
   SUBROUTINE sbc_cpl_rcv( kt, k_fsbc, k_ice )
      ...
      IF( k_ice <= 1 .AND. MOD( kt-1, k_fsbc ) == 0 ) THEN   !  heat & freshwater fluxes ! (Ocean only case)
         ...
         IF( srcv(jpr_sflx )%laction )   sfx(:,:) = -1000.*frcv(jpr_sflx  )%z3(:,:,1) !LB: 1000 because OASIS sends kg/m^2/s while "sfx" in NEMO is 10-3*kg/m^2/s == PSU*kg/m2/s which should == g/m2/s ! + switch sign with respect with neXtSIM.
   ...
   SUBROUTINE sbc_cpl_snd( kt )
      ...
      !
      !
      !  Fields sent by OPA to neXtSIM when doing OPA<->neXtSIM coupling
      !                                                        ! SSH
