...
!CT lateral melting add
   USE limthd_da      ! LIM: lateral melting
!CT lateral melting add
   ...
   SUBROUTINE lim_thd( kt )
      ...
      DO jl = 1, jpl      !loop over ice categories
         ...
         IF( kt == nit000 .AND. lwp ) THEN
            ...
            WRITE(numout,*) ' ~~~~~~~~'
            WRITE(numout,*) '           This version include the lateral melting CALL lim_thd_da'
         ENDIF
...
!CT lateral melting add {
         &                rn_cdsn, nn_monocat, ln_it_qnsice, ln_limdA, rn_beta, rn_dmin
!CT lateral melting add }
      !!-------------------------------------------------------------------
      !
      IF(lwp) THEN
