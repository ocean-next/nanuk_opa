...
!CT to avoid any reduction of background coef. when using new TMX { 
      !
      IF (lwp) WRITE(numout,*) '      Since zdftmx_new is activated, we switched off '
      IF (lwp) WRITE(numout,*) '      the reduction of background diffusion under sea-ice '
      IF (lwp) WRITE(numout,*) '      Setting nn_havti = 1 will have no effect under sea-ice '
      !
!CT to avoid any reduction of background coef. when using new TMX } 
# endif
