...
!{ CREG modifications
   SUBROUTINE fld_read( kt, kn_fsbc, sd, map, kit, kt_offset, jpk_bdy, fvl, ld_startrec  )
!}
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_read  ***
      !!                   
      ...
      INTEGER  , INTENT(in   ), OPTIONAL     ::   kt_offset ! provide fields at time other than "now"
                                                            ...
                                                            !   etc.
!{ CREG modifications
      LOGICAL  , INTENT(in   ), OPTIONAL     ::   ld_startrec
      !!
...
!{ CREG modifications
      ll_startrec = .false.
      IF( PRESENT(ld_startrec) ) ll_startrec = ld_startrec
...
!{ CREG modifications
               IF( PRESENT(jpk_bdy) ) THEN
                  CALL fld_init( kn_fsbc, sd(jf), imap, jpk_bdy, fvl, ld_startrec=ll_startrec  )  ! read each before field (put them in after as they will be swapped)
               ELSE
                  CALL fld_init( kn_fsbc, sd(jf), imap, ld_startrec=ll_startrec )  ! read each before field (put them in after as they will be swapped)
               ENDIF
!}
         END DO
...
!{ CREG modifications
               CALL fld_rec( kn_fsbc, sd(jf), kt_offset = it_offset, kit = kit, ld_startrec=ll_startrec )    ! update after record informations
!}
               ...
               ENDIF
               !LOLO:
               !!GS: add exception due to SAS offset and adapted from ln_tint case (note: should not concern fld_read when called by bdy)
               IF ( nn_components == jp_iam_sas .AND. .NOT. lk_bdy .AND. .NOT. ll_firstcall .AND. sd(jf)%nrec_a(1) > sd(jf)%nreclast ) THEN
                     WRITE(numout,*) 'EM entering new condition'
                     call flush(numout)
                  ...
                  IF( .NOT. ( sd(jf)%ln_clim .AND. sd(jf)%cltype == 'yearly' ) ) THEN   ! close/open the current/new file
                     ...
                     WRITE(numout,*) '!!GS llnxtmth:',llnxtmth
                     WRITE(numout,*) '!!GS llnxtyr:', llnxtyr
                     call flush(numout)
                  ...
                  ENDIF
               ELSE
                     WRITE(numout,*) 'EM standard opening'
                     call flush(numout)
...
!{ CREG modifications
   SUBROUTINE fld_init( kn_fsbc, sdjf, map, jpk_bdy, fvl, ld_startrec )
!}
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_init  ***
      !!
...
!{ CREG modifications
      CALL fld_rec( kn_fsbc, sdjf, ldbefore = .TRUE., ld_startrec=ld_startrec )  ! return before values in sdjf%nrec_a (as we will swap it later)
!}
...
!{ CREG modifications
   SUBROUTINE fld_rec( kn_fsbc, sdjf, ldbefore, kit, kt_offset, ld_startrec )
!}
      !!---------------------------------------------------------------------
      !!                    ***  ROUTINE fld_rec  ***
      !!
...
!}
      ...
      ELSE                                   ! higher frequency mean (in hours)
         ...
         IF( sdjf%ln_tint ) THEN                ! time interpolation, shift by 1/2 record
            ...
            !                   
!{ CREG modifications
            ztmp= ztmp / REAL(ifreq_sec, wp) + 0.5 + COUNT((/ld_startrec/)) * 0.5
!}
         ELSE                                   ! no time interpolation
            !
            !           INT( ztmp/ifreq_sec )
...
!{ CREG modifications
             sdjf%nrec_a(2) = iendrec - ifreq_sec / 2 - COUNT((/ld_startrec/)) * ifreq_sec / 2    ! swap at the middle of the record
!}
         ELSE
...
!{ CREG modifications
      ...
      IF( PRESENT(jpk_bdy) .AND. jpk_bdy>0 ) THEN       ! boundary data not on model grid: vertical interpolation
         ...
         SELECT CASE( igrd )                  
            CASE(1)
!CT                 IF ( lwp )   THEN
!CT                     WRITE(numout,*) " In fldread module"
!CT                     WRITE(numout,*) " dta_read field"
!CT                     WRITE(numout,*) " dta_read(10,1,:) ", dta_read(10,1,:) 
!CT                 ENDIF
...
!CT hard coded fille value
         !fv=0.e0
         WRITE(numout,*) '    Hard coded Fillvalue in subroutine fld_map with value:', fv
!CT hard coded fille value
...
!CT                 IF ( lwp )   THEN
!CT                     WRITE(numout,*) " CHeck vertical interpolation "
!CT                     WRITE(numout,*) " In fldread module"
!CT                     WRITE(numout,*) " dta  interpel"
!CT                     WRITE(numout,*) " igrd  ", igrd
!CT                     WRITE(numout,*) " dta_read(10,1,:)  - dta(10,1,:) - dta_read_z - gdept_0 "
!CT                     DO ik = 1, ipk
!CT                     WRITE(numout,*) " ik ", ik, "  - "
!CT                     WRITE(numout,*)  dta_read(10,1,ik), dta(10,1,ik), dta_read_z(10,1,ik), gdept_0(10,1,ik)
!CT                     ENDDO
!CT                 ENDIF
#endif
      ...
      IF ( map%ll_unstruc ) THEN                            ! unstructured open boundary data file
         ...
         DO ib = 1, ipi
            ...
            DO ik = 1, ipk                      
               SELECT CASE( igrd )                       
                  CASE(1)
                     zl =  gdept_0(zij,zjj,ik)                                          ! if using in step could use fsdept instead of gdept_0?
                  CASE(2)
                     IF(ln_sco) THEN
                        zl =  ( gdept_0(zij,zjj,ik) + gdept_0(zij+1,zjj,ik) ) * 0.5_wp  ! if using in step could use fsdept instead of gdept_0?
                     ELSE
                        zl =  MIN( gdept_0(zij,zjj,ik), gdept_0(zij+1,zjj,ik) ) 
                     ENDIF
                  CASE(3)
                     IF(ln_sco) THEN
                        zl =  ( gdept_0(zij,zjj,ik) + gdept_0(zij,zjj+1,ik) ) * 0.5_wp  ! if using in step could use fsdept instead of gdept_0?
                     ELSE
                        zl =  MIN( gdept_0(zij,zjj,ik), gdept_0(zij,zjj+1,ik) )
                     ENDIF
               END SELECT
         ...
         IF(igrd == 2) THEN                                 ! do we need to adjust the transport term?
            DO ib = 1, ipi
              ...
              zjj = idx_bdy(ibdy)%nbj(ib,igrd)
              zh  = SUM(dta_read_dz(map%ptr(ib),1,:) )
              flagu = idx_bdy(ibdy)%flagu(ib,igrd)
              ztrans = 0._wp
              ztrans_new = 0._wp
              DO ik = 1, jpk_bdy                            ! calculate transport on input grid
                  ztrans     = ztrans     + dta_read(map%ptr(ib),1,ik) * dta_read_dz(map%ptr(ib),1,ik) * ABS(flagu)
              ENDDO
              DO ik = 1, ipk                                ! calculate transport on model grid
                  ztrans_new = ztrans_new + dta(ib,1,ik) * e3u_0(zij,zjj,ik) * ABS(flagu) * umask(zij,zjj,ik)
         ...
         IF(igrd == 3) THEN                                 ! do we need to adjust the transport term?
            DO ib = 1, ipi
              ...
              zjj = idx_bdy(ibdy)%nbj(ib,igrd)
              zh  = SUM(dta_read_dz(map%ptr(ib),1,:) )
              flagv = idx_bdy(ibdy)%flagv(ib,igrd)
              ztrans = 0._wp
              ztrans_new = 0._wp
              DO ik = 1, jpk_bdy                            ! calculate transport on input grid
                  ztrans     = ztrans     + dta_read(map%ptr(ib),1,ik) * dta_read_dz(map%ptr(ib),1,ik) * ABS(flagv)
              ENDDO
              DO ik = 1, ipk                                ! calculate transport on model grid
                  ztrans_new = ztrans_new + dta(ib,1,ik) * e3v_0(zij,zjj,ik) * ABS(flagv) * vmask(zij,zjj,ik)
              ...
              DO ik = 1, ipk                                ! make transport correction
                 ...
                 IF(fvl) THEN ! bdy data are total velocity so adjust bt transport term to match input data
                    dta(ib,1,ik) = ( dta(ib,1,ik) + ( ztrans - ztrans_new ) / hv_0(zij,zjj) ) * vmask(zij,zjj,ik)
                 ELSE ! we're just dealing with bc velocity so bt transport term should sum to zero
                    dta(ib,1,ik) = dta(ib,1,ik) + ( 0._wp - ztrans_new ) / hv_0(zij,zjj) * vmask(zij,zjj,ik)
                 ENDIF
              ENDDO
            ENDDO
         ENDIF
      ...
      ELSE ! structured open boundary file
         ...
         IF(igrd == 2) THEN                                 ! do we need to adjust the transport term?
            DO ib = 1, ipi
              ...
              ztrans_new = 0._wp
              DO ik = 1, jpk_bdy                            ! calculate transport on input grid
                  ztrans = ztrans + dta_read(ji,jj,ik) * dta_read_dz(ji,jj,ik) * ABS(flagu)
              ENDDO
              DO ik = 1, ipk                                ! calculate transport on model grid
                  ztrans_new = ztrans_new + dta(ib,1,ik) * e3u_0(zij,zjj,ik) * ABS(flagu) * umask(zij,zjj,ik)
              ENDDO
!CT start
!CT              IF (lwp .AND. ib == 19 ) THEN
!CT                  WRITE(numout,*) " Compare transport IGRD == 2 on few points"
!CT                  WRITE(numout,*) " ztrans - ztrans_new :", ztrans, ztrans_new
!CT              ENDIF
!CT end
              ...
              DO ik = 1, ipk                                ! make transport correction
                 IF(fvl) THEN ! bdy data are total velocity so adjust bt transport term to match input data
!CT start
!CT                    IF (lwp .AND. ib == 19 .AND. ik <= 10 ) THEN
!CT                        WRITE(numout,*) " ik - before :"
!CT                        WRITE(numout,*) ik, dta(ib,1,ik)
!CT                    ENDIF
!CT end
...
!CT end
         ...
         IF(igrd == 3) THEN                                 ! do we need to adjust the transport term?
           DO ib = 1, ipi
              ...
              ztrans_new = 0._wp
              DO ik = 1, jpk_bdy                            ! calculate transport on input grid
                  ztrans     = ztrans     + dta_read(ji,jj,ik) * dta_read_dz(ji,jj,ik) * ABS(flagv)
              ENDDO
              DO ik = 1, ipk                                ! calculate transport on model grid
                  ztrans_new = ztrans_new + dta(ib,1,ik) * e3v_0(zij,zjj,ik) * ABS(flagv) * vmask(zij,zjj,ik)
              ENDDO
!CT start
!CT              IF (lwp .AND. ib == 19 ) THEN
!CT                  WRITE(numout,*) " Compare transport IGRD == 3 on few points"
!CT                  WRITE(numout,*) " ztrans - ztrans_new :", ztrans, ztrans_new
!CT                  WRITE(numout,*) 
!CT              ENDIF
!CT end
...
!CT start
!CT                    IF (lwp .AND. ib == 19 .AND. ik <= 10 ) THEN
!CT                        WRITE(numout,*) " ik - after  :"
!CT                        WRITE(numout,*) ik, dta(ib,1,ik), hvr(zij,zjj), vmask(zij,zjj,ik)
!CT                    ENDIF
!CT end
                 ELSE ! we're just dealing with bc velocity so bt transport term should sum to zero
...
!{ CREG modifications
         sdf(jf)%igrd     = -1 ! Set to non zero default value to avoid errors, is updated to meaningful value during bdy_dta_init
         sdf(jf)%ibdy     = -1 ! Set to non zero default value to avoid errors, is updated to meaningful value during bdy_dta_init
!}
      END DO
