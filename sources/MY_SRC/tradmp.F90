   ...
   SUBROUTINE resto_patch ( plon1, plon2, plat1, plat2, pbw ,ptmax, presto, pz1, pz2 )
      ...
      IF (PRESENT (pz1) ) THEN 
       ...
       &                         * cos(rad*(plon1-glamt(ji,jj)))
        ...
        zmask(1:ik1) = 1.
!CT bug correction start {
        iloc=MINLOC(zmask) ; ik2 = iloc(1) - 1
!CT        iloc=MAXLOC(zmask) ; ik2 = iloc(1) - 1
!CT        zmask(1:ik1) = 1.
!CT        iloc=MAXLOC(zmask) ; ik2 = iloc(1) - 1
!CT        IF (ik2 > 2 ) THEN
          zmask = 0._wp
        IF (ik2-ik1 > 4 ) THEN
!CT bug correction end }
          zmask(ik1       ) = 0.25_wp
          zmask(ik1+1     ) = 0.75_wp
        ...
        ELSE
!CT bug correction start {
!CT          zmask = 1.   ! all the water column is restored the same
          zmask(ik1:ik2) = 1.   ! all the water column is restored the same
!CT bug correction end }
        ENDIF
