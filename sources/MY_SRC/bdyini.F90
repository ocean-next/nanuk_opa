...
!CT recent bug correction ticket 1734
            jpbdtas = MAX(jpbdtas, (nbdyend - nbdybeg + 1))
!proposed correction line  jpbdtas = MAX(jpbdtas, (nbdyend - nbdybeg + 1)*nn_rimwidth(ib_bdy))
!CT recent bug correction ticket 1734
...
!{ CREG modifications
         ...
         IF ( icount>0 ) THEN
            IF( nb_jpk_bdy>0 ) THEN
!CT recent bug correction ticket 1734
!proposed correction lines               ALLOCATE( dta_global2(jpbdtas, 1, nb_jpk_bdy) )
!proposed correction lines               ALLOCATE( dta_global2_z(jpbdtas, 1, nb_jpk_bdy) )
!proposed correction lines               ALLOCATE( dta_global2_dz(jpbdtas, 1, nb_jpk_bdy) )
!proposed correction lines            ELSE
!proposed correction lines               ALLOCATE( dta_global2(jpbdtas, 1, jpk) )
!proposed correction lines               ALLOCATE( dta_global2_z(jpbdtas, 1, jpk) ) ! needed ?? TODO
!proposed correction lines               ALLOCATE( dta_global2_dz(jpbdtas, 1, jpk) )! needed ?? TODO  
...
!CT         igrd = 2      ! Lateral surface at U-points
!CT         DO ib_bdy = 1, nb_bdy
!CT            DO ib = 1, idx_bdy(ib_bdy)%nblenrim(igrd)
!CT               nbi => idx_bdy(ib_bdy)%nbi(ib,igrd)
!CT               nbj => idx_bdy(ib_bdy)%nbj(ib,igrd)
!CT               flagu => idx_bdy(ib_bdy)%flagu(ib,igrd)
!CT               bdysurftot = bdysurftot + hu     (nbi  , nbj)                           &
!CT                  &                    * e2u    (nbi  , nbj) * ABS( flagu ) &
!CT                  &                    * tmask_i(nbi  , nbj)                           &
!CT                  &                    * tmask_i(nbi+1, nbj)                   
!CT            ENDDO
!CT         ENDDO
...
!CT      DO ib_bdy = 1, nb_bdy
         ...
         IF(lwp) WRITE(numout,*) '               bdysurftot    : ', bdysurftot *1e-6 , '  km2'
!CT add volume correction }
         !
