...
!CT TKE LC effect reduction
                  ...
                  !VH en(ji,jj,jk) = en(ji,jj,jk) + rdt * ( MAX(0.,1._wp - 4.*fr_i(ji,jj) ) ) * ( zwlc * zwlc * zwlc ) /   &
...
!CT TKE injection reduction
                     ...
                     !VH &                                 * ( MAX(0.,1._wp - 4.*fr_i(ji,jj) ) )  * wmask(ji,jj,jk) * tmask(ji,jj,1)
!CT                     &                                 * ( 1._wp - fr_i(ji,jj) )  * wmask(ji,jj,jk) * tmask(ji,jj,1)
               END DO
            END DO
         END DO
...
!CDIR NOVERRCHK
   ...
   SUBROUTINE tke_avn
      ...
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) :: zhice_max   !  VH 13/03/2017
      !!--------------------------------------------------------------------
      !
      IF( nn_timing == 1 )  CALL timing_start('tke_avn')

      CALL wrk_alloc( jpi,jpj,jpk, zmpdl, zmxlm, zmxld ) 
   !!   CALL wrk_alloc( jpi,jpj, zhice_max )    ! VH 13/03/2017
      ALLOCATE(zhice_max(jpi,jpj))
      !                     !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      !                     !  Mixing length
      !                     !<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
      !
...
!{ DRAKKAR 
                  avt(ji,jj,jk)   = MAX( zpdlr * avt(ji,jj,jk), ziceavtb(ji,jj) * avtb(jk) ) * wmask(ji,jj,jk)
...
# endif
      ...
      DEALLOCATE(zhice_max)  ! VH 13/03/2017
      !
...
!CT { TKE LC injection reduction
      IF( ln_lc ) THEN
         IF(lwp) WRITE(numout,*) '       In case ln_lc , reduce the injection of surface TKE arising from LC '
         IF(lwp) WRITE(numout,*) '                      en(ji,jj,jk) = en(ji,jj,jk) + rdt * ( MAX(0.,1._wp - 4.*fr_i(ji,jj) ) ) * ( zwlc * zwlc * zwlc )'
      ENDIF
!CT { TKE LC injection reduction
...
!CT }
         CASE( 2 )                                 ! F(latitude) : 0.5m to 10m poleward of 40 degrees
!CT { TKE injection reduction
